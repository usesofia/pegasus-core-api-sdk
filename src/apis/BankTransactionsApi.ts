...
export interface BankTransactionsApiInterface {
     ...
     * @param {string} providerTransactionId Identificador da movimentação financeira no provedor (providerTransactionId), não o _id do documento.
    ...
    /**
     * 
     * @summary Remove uma movimentação financeira pelo identificador no provedor (providerTransactionId). Se estiver conciliada, desfaz a conciliação antes e notifica o usuário por e-mail.
     * @param {string} providerTransactionId Identificador da movimentação financeira no provedor (providerTransactionId), não o _id do documento.
     * @param {string} ownerOrganizationId Identificador da organização proprietária da movimentação financeira.
     * @param {*} [options] Override http request option.
     * @throws {RequiredError}
...
export class BankTransactionsApi extends runtime.BaseAPI implements BankTransactionsApiInterface {
    ...
    async systemRemoveBankTransactionRequestOpts(requestParameters: SystemRemoveBankTransactionRequest): Promise<runtime.RequestOpts> {
        if (requestParameters['providerTransactionId'] == null) {
            throw new runtime.RequiredError(
                'providerTransactionId',
                'Required parameter "providerTransactionId" was null or undefined when calling systemRemoveBankTransaction().'
            );
        }
        ...
        let urlPath = `/internal/bank-transactions/{providerTransactionId}`;
        urlPath = urlPath.replace(`{${"providerTransactionId"}}`, encodeURIComponent(String(requestParameters['providerTransactionId'])));
